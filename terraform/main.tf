...
# Bootstrap resources for Terraform state storage and locking.
# These are created once and shared across all environments.
# They are named explicitly (not derived from locals) so the
# deploy script can target them during bootstrap.

resource "aws_s3_bucket" "terraform_state" {
  bucket = "broker-terraform-state-${data.aws_caller_identity.current.account_id}"
  tags = {
