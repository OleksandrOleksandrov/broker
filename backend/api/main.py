...
def _compress_single_pdf(
    pdf_bytes: bytes, max_size_kb: int, remove_color: bool
) -> bytes:
    DPI = 150
...
async def compress_pdf(
    ...
    for file in files:
        pdf_bytes = await file.read()
        try:
            compressed_bytes = _compress_single_pdf(
...
async def parse_transport_documents(
    ...
    carrier = (
        ...
        else _normalize_quotes(cmr.carrier.name_and_address)
        if cmr.carrier
        else None
    )
