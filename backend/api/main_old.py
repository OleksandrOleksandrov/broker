...
async def parse_invoice(
    ...
    parse_uktzed: bool = Form(default=True),
):
    ...
    if not file.filename or not file.filename.lower().endswith(".pdf"):
        raise HTTPException(status_code=400, detail="Завантажений файл має бути PDF")

    client = OpenAI(api_key=api_key)
    pdf_bytes = await file.read()

    try:
        # Конвертуємо сторінки PDF у зображення з високою роздільною здатністю (DPI 300+)
        images = convert_from_bytes(pdf_bytes, dpi=600)
    except Exception as e:
        raise HTTPException(
            status_code=400, detail=f"Помилка зчитування PDF: {str(e)}"
        ) from e
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"OpenAI invoice parsing failed: {str(exc)}",
        ) from exc

    parsed_data = completion.choices[0].message.parsed

    # Підбираємо код УКТ ЗЕД для кожної позиції лише якщо це увімкнено
    if parse_uktzed:
        for item in parsed_data.items:
            try:
                ...
                item.uktzed_suggestion = UktZedSuggestion(
                    ...
                    justification=f"Помилка при запиті класифікації: {str(exc)}",
                )
...
async def export_excel(data: InvoiceData):
    ...
    for idx, item in enumerate(data.items, 1):
        # Витягуємо код УКТ ЗЕД, якщо він є
